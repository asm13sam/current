...
class ProductWidget(QWidget):
    ...
    def form_size_changed(self, width, length, pieces, number):
        ...
        #     self.form.widgets['price'].setValue(p2o['price'])
        #     # profit = round(total * self.form.widgets['persent'].value() / 100 / number, 2)
        #     self.form.widgets['profit'].set_value(p2o['profit'])
        #     self.form.widgets['cost'].set_value(p2o['cost'])
        # self.pw.reload()

    def product_changed(self):
        # self.pw.reload()
        persent = self.form.widgets['persent'].value()
        print('form', self.product.value['product_extra']['product_to_ordering']['cost'])
