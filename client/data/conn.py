...
class Data:
    ...
    def get_all(self, model_name, all=''):
    ...
    def get_filter(self, model_name, filter_field, filter_value, all=''):
    ...
    def get_between(self, model_name, field, value1, value2, all=''):
    ...
    def get_sum_before(self, model_name, sum_field, field, id, date_before):
        url = f'{self.base_url}{model_name}_of_{sum_field}_sum_before/{field}/{id}/{date_before}'
        # print('GET_SUM_BEFORE', url)
        response = requests.get(url, cookies=self.cookie_jar)
        return self.format_response(response)

    def get_sum_filter(self, model_name, field1, id1, field2, id2):
    ...
    def get_find(self, model_name, find, find_value):
        find_pref = ''
...
# --------------------------WWWWWWWWWWWWWWW----------------------------
    ...
    def get_w(self, model_name, id):
        url = f'{self.base_url}w_{model_name}/{id}'
        # print('WGET', url)
        response = requests.get(url, cookies=self.cookie_jar)
        return self.format_response(response)

    def get_all_w(self, model_name, all=""):
    ...
    def get_filter_w(self, model_name, filter_field, filter_value, all=''):
        filter_type = 'int' if type(filter_value) is int else 'str'
    ...
    def get_between_w(self, model_name, field, value1, value2, all=''):
    ...
    def get_between_up_w(self, model_name, field, value1, value2, all=''):
    ...
    # "find": [{"project": ["info"], "contragent":["-search"], "contact":["-search"]}]
    # "/find_project_project_info_contragent_no_search_contact_no_search/{fs}"
    def get_find_w(self, model_name, find, find_value):
        find_pref = ''
        for k, v in find.items():
            find_pref += f"_{k}"
            for i in v:
